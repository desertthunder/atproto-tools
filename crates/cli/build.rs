...
fn main() {
    let workspace = PathBuf::from(std::env::var_os("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR is set"))
        ...
        .to_path_buf();
    let docs_path = workspace.join("docs").join("cli.txt");
    let manual = render_manual(args::Cli::command()).expect("render CLI manual");
...
fn render_manual(command: clap::Command) -> std::io::Result<String> {
    ...
    render_command(command, vec!["atp".to_string()], &mut rendered)?;
    Ok(rendered)
}
